"""
Flask app for annabelle project
"""
...
IMG_SIZE = 224
CLASSES = ['No Fracture', 'Greenstick', 'Torus', 'Transverse', 'Oblique', 'Spiral', 'Comminuted', 'Salter-Harris']
...
def get_test_samples():
    ...
    if not os.path.exists(TEST_DATA_DIR):
        return [
            {'id': f'sample_{i}', 'path': f'test/{CLASSES[i % 8].lower().replace(" ", "_")}/img_{i}.jpg', 'label': CLASSES[i % 8]}
            for i in range(30)
        ]
...
def predict():
    ...
    if not os.path.exists(TEST_DATA_DIR):
        import random
        true_label = CLASSES[random.randint(0, 7)]
        pred_idx = random.randint(0, 7)
        pred_label = CLASSES[pred_idx]
        probs = [random.random() for _ in range(8)]
        probs[pred_idx] = max(probs) + 0.3
        total = sum(probs)
        probabilities = {CLASSES[i]: float(probs[i] / total) for i in range(8)}
