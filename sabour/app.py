...
def get_test_samples():
    ...
    if not os.path.exists(TEST_DATA_DIR):
        return [
            {'id': f'sample_{i}', 'path': f'test/{CLASSES[i % 2].lower().replace(" ", "_")}/img_{i}.jpg', 'label': CLASSES[i % 2]}
            for i in range(30)
        ]
...
def predict():
    ...
    if not os.path.exists(TEST_DATA_DIR):
        import random
        true_label = CLASSES[random.randint(0, 1)]
        pred_idx = random.randint(0, 1)
        pred_label = CLASSES[pred_idx]
        probs = [random.random() for _ in range(2)]
        probs[pred_idx] = max(probs) + 0.3
        total = sum(probs)
        probabilities = {CLASSES[i]: float(probs[i] / total) for i in range(2)}
